...
def search_reports_query(db: Session, q: str = "", date_from: str = "", date_to: str = "", *, project_id: int) -> Query:
    """管理画面の一覧と同じ条件でレポートを絞り込むクエリを返す（並び順は呼び出し側で付ける）。

    project_id は必須。全プロジェクト横断の検索は提供しない（呼び出し側の指定忘れで
    テナントを跨いだ結果が返るのを防ぐ）。
    q は全テキストフィールドへの ILIKE 部分一致。日付は YYYY-MM-DD で、date_to はその日を含む。
    形式が不正な日付は無視する（画面の挙動と同じ）。
    """
    query = db.query(ReportData).filter(ReportData.project_id == project_id)
