...
│   │   ├── google_auth.py  # Google ログイン（/auth/google, コールバック）
│   │   ├── invite.py       # 招待リンクの有効化ページ（/invite/{token}: Google またはパスワード設定）
...
│   │   ├── login.html
│   │   ├── invite_accept.html     # アカウントの有効化（招待リンク先）
...
- **メンバー管理**: 新しいメンバーの招待（メールアドレス + 役割）、既存ユーザーの追加、役割変更、除外（プロジェクト管理者）
- **アカウントの有効化**: 招待リンク先（`/invite/{token}`）。本人が Google ログインかユーザー名 + パスワードの設定を選ぶ（ログイン不要）
...
- **全ユーザー管理**: 新しいユーザーの招待（メールアドレス + 初期プロジェクト・システム管理者フラグ）、パスワード直接発行（緊急用）、削除、有効/無効、システム管理者権限、PW変更、Google 連携（システム管理者）
